...
impl<const K: usize> DecisionTree::<K> {
    ...
    pub fn partitions(
        ...
        loop {
            ...
            rule_count = partitions
                .iter()
                .map(|x| x.rules.len())
                .sum();

            println!("opc: check x={} bound={} goal={} rules={} parts={}", 
                x, bound, goal, rule_count, partitions.len());
    ...
    fn partition(
        ...
        for partition in 0..count {
            ...
            let mut p = Partition::<K>::new(
                Interval::new(p_begin, p_end),
                Vec::new(),
            );
...
mod tests {
    ...
    fn rules_from_paper() -> Vec::<Rule::<2>> {
        vec![
            Rule::new(
                "r1", [Interval::new(0, 31), Interval::new(0, 255)]
            ),
            Rule::new(
                "r2", [Interval::new(0, 255), Interval::new(128, 131)]
            ),
            Rule::new(
                "r3", [Interval::new(64, 71), Interval::new(128, 255)]
            ),
            Rule::new(
                "r4", [Interval::new(67, 67), Interval::new(0, 127)]
            ),
            Rule::new(
                "r5", [Interval::new(64, 71), Interval::new(0, 15)]
            ),
            Rule::new(
                "r6", [Interval::new(128, 191), Interval::new(4, 131)]
            ),
            Rule::new(
                "r7", [Interval::new(192, 192), Interval::new(0, 255)]
            ),
        ]
    }
    ...
    fn partition_rule_count() {
        ...
        let partitions = DecisionTree::<2>::partition(
            &rules,
            0,
            0,
            4,
        ...
        let partitioned_rule_count: usize = partitions
            .iter()
            .map(|x| x.rules.len())
            .sum();
    ...
    fn optimize_partitions() {
        ...
        let (optimal_partitions, partitions) = DecisionTree::<2>::partitions(
            0,
            1.5,
            &rules,
            0,
